...
local feedJobScenarioCoords = { x=-275.11, y=653.79, z=113.42 }
...
Citizen.CreateThread(function()
    ...
    -- local feedJob = CreateStartEndScenarion(
        -- 'Alimentar galinhas', 'Encontre algumas galinhas para alimentar.',
    ...
    local feedJob2 = CreateStartEndScenarion(
        'Alimentar galinhas 2', 'Coloca alguma comida as galinhas.',
        GetHashKey('WORLD_PLAYER_CHORES_FEEDBAG_PICKUP'), GetHashKey('WORLD_PLAYER_CHORES_FEEDBAG_PUTDOWN'),
    ...
    AddEventHandler('immersion:job_ended:' .. feedJob2, function(_scenario)
		...
		Citizen.Wait(2000)
		if feedJob2Completed == true then
		TriggerServerEvent("alimentar:galinhas")
		end
    end)
    ...
    while true do
        ...
        if feedJob2Scenario then
            ...
            if not feedJob2Completed and DoesScenarioExistInArea(feedJob2Coord.x, feedJob2Coord.y, feedJob2Coord.z, 1.0) == 1 then
                DrawText3D(feedJob2Coord.x, feedJob2Coord.y, feedJob2Coord.z, 'Alimentar Galinhas')
            end
            ...
            elseif feedJob2Completed == true then
                DrawText3D(feedJob2Data.coord.x, feedJob2Data.coord.y, feedJob2Data.coord.z, 'Volta alimentar')
            end
        end
    end
    -- END Feed Job 2
...
-- Citizen.CreateThread(function()
    -- while true do
        ...
        -- if feedJobActive then
            -- DrawText3D(feedJobData.coord.x, feedJobData.coord.y, feedJobData.coord.z, 'Volta alimentar')
            -- local player = PlayerPedId()
